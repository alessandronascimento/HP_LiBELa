...
int main()
{
    ...
    printf ("#Thermo: %10.5s %10.5s %10s %10s %10s %10s\n", "T", "lnQ", "U", "S", "F", "CV");
    for (unsigned i=0; i<vlnQ.size()-1; i++){
        printf("Thermo: %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f\n", vt[i], vlnQ[i], (k*vt[i]*vt[i]*dlnQdT[i]), ((k*vlnQ[i])+(vkt[i]*dlnQdT[i])), (-vkt[i]*vlnQ[i]), dUdT[i]);
    }

    printf("#Probs: %10s %10s %10s %10s %10s\n", "T", "-8.0", "-5,0", "-3.5", "-2.0");
    for (unsigned i=0; i<vQ.size()-1; i++){
        printf("Probs: %10.5f %10.5f %10.5f %10.5f %10.5f\n", vt[i], (exp(8/vkt[i])/vQ[i]), (exp(5/vkt[i])/vQ[i]), (exp(3.5/vkt[i])/vQ[i]), (exp(2/vkt[i])/vQ[i]));
    }
